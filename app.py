...
<style>
    ...
    .amazon-btn {
        background: #3B82F6; color: white !important; padding: 22px; width: 100%; display: block; 
        text-align: center; border-radius: 12px; font-weight: 900; text-decoration: none; font-size: 1.4em;
...
def get_expert_verdict(row, mode):
    ...
    if "Gamer" in mode: verdict += f"Dominates the arena with an <b>AnTuTu score of {int(row['antutu']):,}</b>. Verified as a top-tier gaming powerhouse for 2025."
